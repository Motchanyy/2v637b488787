...
(function (window, document) {
	...
	function initLoader() {
		...
		//    (розкриваються над головною кнопкою +/×)
		//    Кожен елемент: { label, href АБО onClick, bg, svg }
		// ─────────────────────────────────────────────────────────
		const EXTRA_BUTTONS = [
			{ label: "Telegram", href: "https://t.me/skyneuron", bg: "#29b6f6", svg: '<svg viewBox="0 0 24 24"><path d="M9.8 15.6 9.6 19c.4 0 .6-.2.8-.4l1.9-1.8 3.9 2.9c.7.4 1.2.2 1.4-.7l2.6-12c.2-1-.3-1.5-1-1.2L3.5 10.2c-1 .4-1 .9-.2 1.2l4.2 1.3L17 6.6c.5-.3.9-.1.6.2l-7.8 8.8z"/></svg>' },
			{ label: "Email", href: "mailto:manager@skyneuron.com", bg: "#f4511e", svg: '<svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z"/></svg>' },
		];
    ...
    `;
      ...
      <svg class="mfb-component__main-icon--active" viewBox="0 0 24 24"><path d="M18.3 5.7 12 12l6.3 6.3-1.4 1.4L10.6 13.4 4.3 19.7 2.9 18.3 9.2 12 2.9 5.7 4.3 4.3l6.3 6.3 6.3-6.3z"/></svg>`;
		...
		// ── Тогл меню (працює і на тач, і на клік) ──
		// stopPropagation — щоб документний обробник (клік поза меню)
		// не спрацьовував одразу після відкриття і не закривав його.
		mainBtn.addEventListener("click", (e) => {
			e.preventDefault();
			e.stopPropagation();
			if (mfbWrap.classList.contains("lc-busy")) return; // чат відкритий — меню не чіпаємо
		...
		// ─────────────────────────────────────────────────────────
		// 8. Ховер на десктопі (тільки там, де він справжній).
		//    На тач-пристроях ці події не спрацьовують — там лише клік.
		// ─────────────────────────────────────────────────────────
